...
exoN=10.^(minE+(maxE-minE)*parg(:,10));
X0g=[u0,T1,T2,T3,exoN];
Ka_UT1=10.^(minKa+(maxKa-minKa)*parg(:,5))/10^9;
Ka_aT1=10.^(minKa+(maxKa-minKa)*parg(:,6))/10^9;
Ka_YT3=10.^(minKa+(maxKa-minKa)*parg(:,9))/10^9;
Ka_inhT3=10.^(minKa+(maxKa-minKa)*parg(:,8))/10^9;
Ka_inhT2=(1+(5-1)*parg(:,7)).*Ka_inhT3;
Kag=[Ka_UT1,Ka_aT1,Ka_inhT2,Ka_YT3,Ka_inhT3];
...
for majori = 1:maxmajor
    ...
    logks = newgoods(:,vark);
    % Perform principal component analysis for those samples whose variance is
    % not 0.
    ...
    allgoodcond{majori}=goodconditions;
    clear newgoods goodconditions
    ...
    for i = 1:size(sets,1)
        ...
        % Additionally, inhibitor binds stronger to T2 than to T3
        % (dG(3)<dG(5)).
        deltaG=-Rc*T*log(sets(i,6:10)*10^9)-Rc*T*log(55);
        if min(sets(i,1:4))>=0.1&&max(sets(i,1:4))<=100&&sets(i,5)>=10&&sets(i,5)<=100&&min(deltaG([1,2,3,4,5]))>=-25&&max(deltaG([1,2,3,4,5]))<=-8&&deltaG(3)<deltaG(5)
            try
