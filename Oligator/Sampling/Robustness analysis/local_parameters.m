...
for i=1:1
    ...
    % deltaG > -8 makes no sense practically.
    validdG=deltaG(deltaG(:,3)>deltaG(:,4),:);
    validconc=conc(deltaG(:,3)>deltaG(:,4),:);
    clear deltaG conc
    deltaG=validdG(indmax(1),:);
    deltaG=[deltaG(1) deltaG(1) deltaG(2) deltaG(2) deltaG(3) deltaG(4)];
    conc=validconc(indmax(1),:);
end
Rholocal=zeros(1,6);
